...
bot.onText(/\/start|Вернуться в начало ↩️/, (msg) => {
  ...
  userState[chatId] = { step: "/start", data: {} };require("dotenv").config();
...
const bot = new TelegramBot(BOT_TOKEN, { polling: true }); // Инициализация бота
...
bot.onText(/\/start|Вернуться в начало ↩️/, (msg) => {
  ...
  // Отправляем информацию об условиях работы без кнопок
  bot.sendMessage(
    chatId,
...
bot.on("message", (msg) => {
  ...
  // Сначала отправляем сообщение с информацией о компании
  bot.sendMessage(
    chatId,
    "🚴🏼‍♂️ Компания Самокат приглашает вас на работу!\n\n" +
    "У нас есть две актуальные вакансии:\n" +
